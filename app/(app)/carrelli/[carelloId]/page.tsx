...
import { CarrelloDetailView } from '@/components/carrello-detail-view';
import { getUoAttivaId } from '@/lib/uo-cookie';
import type { CurrentUserContext } from '@/lib/types';

export default async function CarrelloDetailPage({
  params,
}: {
  params: Promise<{ carelloId: string }>;
}) {
  const { carelloId } = await params;
  ...
  const [unitaRes, carrelloRes, articoliRes] = await Promise.all([
    ...
    supabase
      ...
      .select('*')
      .eq('carrello_id', carelloId)
      .order('nome_articolo'),
  ]);

  if (carrelloRes.error || !carrelloRes.data) notFound();

  const unita = unitaRes.data ?? [];
  const uoAttiva = unita.find((u: { id: string }) => u.id === uoAttivaId) ?? null;
  const carrello = carrelloRes.data;
  ...
  return (
    <div className="min-h-screen bg-bg">
      ...
      <main className="max-w-3xl mx-auto px-4 py-8">
        <div className="mb-6">
          <BackButton />
          <h1 className="font-display text-3xl font-semibold text-ink mt-2">{carrello.nome}</h1>
