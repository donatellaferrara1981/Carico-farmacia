...
// Alias inglese → italiano per unificare nomi estratti da PDF
const ALIAS_NOMI: Record<string, string> = {
  ...
  'fluconazole':     'Fluconazolo',
  'omeprazole':      'Omeprazolo',
  'pantoprazole':    'Pantoprazolo',
  'lansoprazole':    'Lansoprazolo',
  ...
  'bisoprolol':      'Bisoprololo',
  'metoprolol':      'Metoprololo',
  'carvedilol':      'Carvedilolo',
  ...
  'codeine':         'Codeina',
  'codeina':         'Codeina',
  'oxycodone':       'Ossicodone',
  'hydromorphone':   'Idromorfone',
  'methadone':       'Metadone',
  'naloxone':        'Naloxone',
...
function normalizzaNome(s: string): string {
  const chiave = s.toLowerCase().trim();
  if (ALIAS_NOMI[chiave]) return ALIAS_NOMI[chiave];
  return s
    .split(' ')
